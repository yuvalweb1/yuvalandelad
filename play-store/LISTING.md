# Google Play Console — Listing Copy & Forms

This file contains every text field you'll fill in the Play Console store
listing, in both English and Hebrew, plus the answers for the Data Safety
questionnaire. Copy-paste from here.

> **Sources of truth.** The "100% local" claim must match the [Privacy Policy](https://yuval.ella.org.il/privacy.html). If you change one, change the other.
...
## Main Store Listing — Hebrew (he-IL)
...
| 7-inch tablet (optional) | min 1024×600 | ❌ Skip for v1 | — |
| 10-inch tablet (optional) | min 1080×1920 | ❌ Skip for v1 | — |
| Promo video (optional) | YouTube URL | ❌ Skip for v1 | — |

**Screenshots — recommended capture targets:**
1. Landing page hero ("ready to expose your group?")
2. A leaderboard slide
3. A roast card (the Verdict screen)
4. A signature-words / per-person stat slide
5. The PostMenu hub
6. The Premium promo modal (proves the monetization story to reviewers)

Take 6 from the iPhone in Safari (Add to Home Screen → full-screen mode) or from an Android emulator. Min height ~1280px is fine; portrait orientation matches the 9:16 frame.

**Feature graphic** — a 1024×500 banner shown above the screenshots. Needs
a small design (logo + tagline on banana/mango gradient). Can be generated
with the same script later, or designed in Figma/Canva.
...
### Data types collected — full list

For each category below, the answer is **"Data type not collected"**:

- Personal info (name, email, address, phone, race, etc.)
- Financial info (purchase history, credit card info, etc.) — *In-app subscription billing goes through Google Play Billing. Google handles the payment data, not us. This is exempt from "collected" per Play policy.*
- Health and fitness
- Messages (SMS, email, IM)
- Photos and videos — *The user picks a WhatsApp .zip from their device. We extract photos/voice/videos and process them locally. We do not transmit, store on a server, or share them.*
- Audio files
- Files and docs
- Calendar
- Contacts
...
> **No account.** Users use the app anonymously.
...
## App content & policy questionnaire

| Question | Answer |
|---|---|
| Target audience | Adults (18+) — *recommended to avoid kids-safe scrutiny* |
| News app | No |
| COVID-19 contact tracing | No |
| Government app | No |
| Financial features | No (Premium subscription is non-financial) |
| Health features | No |
...
Pick **IARC**. Answer all questions "No" — there's no violence, sexual
content, gambling, or strong language in the UI (the roasts use mild
sarcasm, comparable to a Spotify Wrapped recap — should still land at
rated 3+ or "Everyone" / PEGI 3).

---

## Release tracks — recommended order

1. **Internal testing** (you + 5-10 friends): upload the AAB, distribute via
   Play Store internal track. Test the install, IAP flow, share targets,
   parsing, deep links.
2. **Closed testing** (50-100 testers): broader real-world. Iterate.
3. **Open testing** (anyone with the link): pre-launch promotion.
4. **Production**: full Play Store listing live.

Don't skip directly to production for v1 — internal testing catches
99% of crashes before reviewers see them, which keeps your "rejected"
counter clean.

---

## Pre-submission checklist (must all be ✓ before clicking Publish)

- [ ] HTTPS is live on `yuval.ella.org.il` (so the Privacy Policy URL works)
- [ ] Play Developer account created + verified
- [ ] Bank account / Merchant account configured for payouts
- [ ] Keystore generated, `android/keystore.properties` filled in
- [ ] `versionCode` bumped (from `1` to `1` on first push; bump per release)
- [ ] `./gradlew bundleRelease` produces a signed `app-release.aab`
- [ ] App icons (mipmaps) regenerated via `node scripts/generate-icons.mjs`
- [ ] 6 portrait screenshots captured
- [ ] Feature graphic 1024×500 designed
- [ ] Listing copy pasted from this file
- [ ] Data Safety form filled per this file
